 ...
 */
class FormaticModelformatics extends JModelList {
    ...
    protected function populateState($ordering = null, $direction = null) {
        ...
        // Load the parameters.
        $params = JComponentHelper::getParams('com_formatic');
    ...
    protected function getListQuery() {
        ...
        $query->from('`#__formatic` AS a');

        
        // Join over the users for the checked out user.
        $query->select('cat.name AS catname');
        $query->join('LEFT', '#__formatic_categories AS cat ON cat.id=a.category');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.name LIKE '.$search.'  OR  a.category LIKE '.$search.' )');
            }
        }        
